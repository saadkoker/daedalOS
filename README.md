...
```
docker build -t kokerOS .
docker run -dp 3000:3000 --rm --name kokerOS kokerOS
```

##### Notes

- If during `yarn install` you receive the error `digital envelope routines::unsupported`, you need to set `NODE_OPTIONS` to `--openssl-legacy-provider` ([1](https://github.com/DustinBrett/kokerOS/blob/main/Dockerfile#L3), [2](https://github.com/DustinBrett/kokerOS/blob/main/.github/workflows/main.yml#L17), [3](https://stackoverflow.com/a/69699772/5895982))
